...
export function TableBottomNavigation({ tableId }: { tableId: number }) {
  ...
  const items = [
    { href: `${tableRoot}/menu`, label: "المينيو", icon: "menu" },
...
function NavIcon({ name }: { name: "menu" | "cart" | "track" | "service" }) {
  if (name === "menu") return <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="3" width="16" height="18" rx="2" /><path d="M8 8h8M8 12h8M8 16h5" /></svg>;
