-- Never attach a customer to a session that already has a paid payment.
-- A payment_pending session without a paid payment remains orderable.
...
begin
  ...
  select t.cafe_id, t.table_number
    into v_cafe_id, v_table_number
  from public.cafe_tables t
  where t.id = p_table_id
  for update;

  if not found then
    raise exception 'Table was not found' using errcode = 'P0002';
  end if;
  ...
  where s.table_id = p_table_id
    and (
      ...
      or (
        ...
        and not public.session_has_paid_payment(s.id)
      )
    )
