...
begin
  ...
  -- Cashiers may correct active preparation states in either direction and may
  -- confirm physical delivery directly from any active state.
  if v_item.status in ('received','preparing','ready')
     and p_status in ('received','preparing','ready','served','cancelled') then
    null;
  else
