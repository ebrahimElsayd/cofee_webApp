alter table public.cafe_settings
  add column if not exists service_charge_type text not null default 'percent',
  add column if not exists service_charge_value numeric(12,2) not null default 0,
  add column if not exists service_charge_enabled boolean not null default false,
  add column if not exists tax_charge_type text not null default 'percent',
  add column if not exists tax_charge_value numeric(12,2) not null default 0,
  add column if not exists tax_charge_enabled boolean not null default false;

update public.cafe_settings
set service_charge_value = coalesce(service_percentage, 0),
    service_charge_enabled = coalesce(service_percentage, 0) > 0,
    tax_charge_value = coalesce(tax_percentage, 0),
    tax_charge_enabled = coalesce(tax_percentage, 0) > 0
where service_charge_value = 0 and tax_charge_value = 0;
...
begin
  ...
  from public.orders where session_id=p_session_id and status<>'cancelled' and payment_status<>'paid';
